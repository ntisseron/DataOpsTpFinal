...
def extract_boston_salary(url: str = DEFAULT_URL) -> pd.DataFrame:
    ...
    # Normalisation de colonnes utiles si elles existent. PS : Si absentes on laisse le transform gérer
    return df
...
def transform(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # On supprime les lignes sans salaire
    df = df.dropna(subset=["TOTAL_EARNINGS_CLEAN"])
    df = df[df["TOTAL_EARNINGS_CLEAN"] >= 0]
 
    # Conserver uniquement colonnes utiles + id si il est présent
